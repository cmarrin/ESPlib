...
class HTTPParser
{
  public:
    ...
    const std::string getHTTPArg(const char* name) { return _args.count(name) ? _args[name] : ""; }
    const std::string getHTTPHeader(const char* name) { return _headers.count(name) ? _headers[name] : ""; }
